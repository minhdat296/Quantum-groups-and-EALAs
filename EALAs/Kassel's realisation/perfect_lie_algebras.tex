    ...
    \subsection{Structure of finite-dimensional simple Lie algebras} \label{subsection: finite_dimensional_simple_lie_algebras}
        ...
        \begin{example}[$\sl_2$] \label{example: sl_2}
            Recall that $\sl_2(k)$ is the kernel of the trace map:
                $$\trace: \gl_2(k) \to k$$
            i.e. it is the Lie algebra of trace-zero $2 \x 2$-matrices whose Lie bracket is the usual commutator of matrices. It is of dimension $\dim_{\bbC} \gl_2(k) - \dim_{\bbC} k = 4 - 1 = 3$, and a common choice of basis is:
        ...
        \end{proposition}
            \begin{proof}
                Suppose firstly that $\t$ is a $\d$-module, say defined by a Lie algebra homomorphism:
                    $$\rho: \d \to \gl(\t)$$
                Also, pick a linear section:
                    $$\gamma: \d \to \frake$$
                Given $K, K' \in \t$ and $D, D' \in \d$, one then has that:
                    $$
                        \begin{aligned}

\section{Some generalities on Lie algebras}
    ...
    \subsection{Structure of finite-dimensional simple Lie algebras} \label{subsection: finite_dimensional_simple_lie_algebras}
        ...
        \begin{theorem}[Serre's Theorem]
            ...
            This is usually referred to as the \textbf{Chevalley-Serre} presentation for $\g$.
        \end{theorem}
        ...
        \begin{example}[$\sl_2$] \label{example: sl_2}
            ...
            and then seeing that because the first element is not nilpotent whereas the other two elements are nilpotent, any Cartan subalgebra of $\sl_2(k)$ must therefore be isomorphic to $k \begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix}$. One can then show that $x^+ = \begin{pmatrix} 0 & 1 \\ 0 & 0 \end{pmatrix}$ and $x^- = \begin{pmatrix} 0 & 0 \\ 1 & 0 \end{pmatrix}$ necessarily, if the Chevalley-Serre relations are to be satisfied. 
    ...
    \subsection{Perfect Lie algebras and their central extensions}
        ...
        \begin{proposition}[Lie brackets on extensions] \label{prop: lie_brackets_on_extensions}
            Suppose that:
                $$0 \to \z \to \frake \to \a \to 0$$
            is an extension of Lie algebras. Then, up to a choice of so-called \say{$2$-cocycle} $\sigma$ (of $\a$ with coefficients in $\z$), which can be regarded as a $k$-linear map:

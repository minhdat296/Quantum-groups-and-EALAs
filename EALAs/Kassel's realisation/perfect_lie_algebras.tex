\section{Some generalities on Lie algebras}
    ...
    \subsection{Some generalities on Lie algebra cohomology}
        ...
        \begin{definition}[Complexes and cohomomology] \label{def: complexes_and_cohomology}
            ...
            The connecting maps $\del_i$ are typically called \textbf{differentials} or \textbf{boundary maps}.
        ...
        \begin{definition}[Lie algebra cohomology and the Chevalley-Eilenberg resolution] \label{def: lie_algebra_cohomology}
            ...
            and for each $i \in \Z_{\geq 0}$, the corresponding differential/boundary map:
            ...
            and observe that its terms $C_{-i}(\a, M)$ can be described as follows:
                $$
                    \begin{aligned}
                        ...
                        := & \Hom_{\rmU(\a)}(k_{-i}(\a), M)
                        \\
                        \cong & \Hom_{\rmU(\a)}( \rmU(\a) \tensor_k \bigwedge^i \a, M ) \ni \left( \varphi: a \tensor (x_1 \wedge ... \wedge x_i) \mapsto \varphi( a \tensor (x_1 \wedge ... \wedge x_i) ) \in M \right)
                        \\
                        \cong & \Hom_{\rmU(\a)}( \rmU(\a), \Hom_k( \bigwedge^i \a, M ) ) \ni \left( \varphi( a \tensor (-) ): x_1 \wedge ... \wedge x_n \mapsto \varphi( a \tensor (x_1 \wedge ... \wedge x_i) ) \in M \right)
                        \\
                        \cong & \Hom_k( \bigwedge^i \a, M ) \ni \left( \right)
                    \end{aligned}
                $$
            for all $a \in \rmU(\a)$ and all $x_1, ..., x_n \in \a$.
        \end{remark}
        Let us also write:
            $$d_{-(i + 1)}: C_{-(i + 1)}(\a, M) \to C_{-i}(\a, M)$$
        for the differentials of thee complex of $k$-vector spaces $\{C_{-i}(\a, M)\}_{i \in \Z_{\geq 0}}$.
        ...
        \begin{remark}
            ...
            Now, recall that in definition \ref{def: twisted_semi_direct_products}, we have already encountered the case $i = 2$; namely, we have defined Lie $2$-cocycles 
        \end{remark}
        ...
        \begin{theorem}[$H^{\Lie}$ and abelian extensions] \label{theorem: H^2_and_abelian_extensions}

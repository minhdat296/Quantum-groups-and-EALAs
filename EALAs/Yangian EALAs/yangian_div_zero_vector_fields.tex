\section{Constructing Yangian extended toroidal Lie algebras}
    \subsection{Extending \texorpdfstring{$\toroidal$}{} by derivations to fix degeneracy}
        ...
        \begin{lemma}[$\d_{[2]}$ acts on $\g_{[2]}$ by derivations] \label{lemma: derivation_action_on_multiloop_algebras}
            ...
            Furthermore, the basis elements of $\d_{[2]}$ are given explicitly in terms of the partial derivatives $\del_v := \frac{\del}{\del v}$ and $\del_t := \frac{\del}{\del t}$ by:
                ...
                $$D_t = -\del_t$$
        \end{lemma}
        \todo[inline]{Fixed the sign errors in the value of $D_{r, s}$.}
        \todo[inline]{\textbf{Reminder:} Dr. Weekes and Dr. Wendlandt are using $\scrD_{r, s} := D_{-r, -s - 1}$. This notation has the advantage that $\deg K_{r, s} = \deg \scrD_{r, s} = s$.}
            \begin{proof}
    ...
    \subsection{Yangian extended toroidal Lie algebras are twisted semi-direct products}
        ...
        \todo[inline]{Fixed $[D_{r, s}, K_{a, b}]_{\extendedtoroidal}$ and $[D_t, K_{a, b}]_{\extendedtoroidal}$.}

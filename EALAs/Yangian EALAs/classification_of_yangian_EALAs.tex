\section{Classification of Yangian extended toroidal Lie algebras}
    ...
    \subsection{Which extended toroidal Lie algebras are Yangian ?}
        ...
        \begin{lemma}[A copy of the Witt algebra inside $\d_{[2]}$]
            ...
            given by:
                $$d_r \mapsto D_{r, -1}$$
            This identifies a copy of $\der(\bbC[v^{\pm 1}])$ - the \textbf{Witt algebra} - inside $\d_{[2]}$ as a Lie subalgebra. 
        \end{lemma}

        The Lie algebra $\der(\bbC[v^{\pm 1}])$ is known to possess a \textit{non-trivial} UCE:
            $$\frakv := \der(\bbC[v^{\pm 1}]) \oplus^{\eta} \bbC c_{\frakv}$$
        called the \textbf{Virasoro algebra}, whose corresponding \textit{non-zero} $2$-cocycle:
        ...
        for all $r \in \Z$. But this is certainly not true for all $r \in \Z$: e.g. if $r = 1$ then we will get that:
            $$-c_v = -v^{-1} \bar{d}v = 0$$
        which is absurd! As such, neither of the $2$-cocycles $\sigma_1$ and $\sigma_2$ coincide with $\eta$ when restricted down to $\frakv := ( \bigoplus_{r \in \Z} \bbC D_{r, -1} ) \oplus \bbC c_{\frakv}$.  
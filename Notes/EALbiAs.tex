...
\begin{document}
    ...
    \section{Full toroidal Lie bialgebras}
        ...
        \subsection{Failing to extend the Lie bialgebra structure to \texorpdfstring{$\uce( \g[v^{\pm 1}, t] )$}{}}
            ...
            \begin{remark}[What exactly is $\z(\s^{\star})$ ?] \label{remark: centres_of_dual_toroidal_lie_bialgebras}
                ...
                This will become useful later on for the purposes of proposition \ref{prop: extended_toroidal_lie_algebras}.
        ...
        \subsection{Extending \texorpdfstring{$ \uce(\g[v^{\pm}, t]) $}{} to fix degeneracy; full toroidal Lie algebras}
            \begin{remark}[Extending $\t$ into a larger vector space] \label{remark: extending_underlying_vector_spaces_of_toroidal_lie_algebras}
                ...
                To begin, let us introduce a vector space extension:
                    $$0 \to \t \to \hat{\t} \to \d \to 0$$
                Since we know that:
                    $$\z(\t) \cong ( \bigoplus_{(r, s) \in \Z^2} \bbC Z_{r, s}) \oplus \bbC c_v \oplus \bbC c_t$$
                with notations as in remark \ref{remark: centres_of_dual_toroidal_lie_bialgebras}, and since we are extending $\t$ by $\d$ simply so we can extend the invariant inner product $(-, -)_{\t}$ to all of $\hat{\t}$ to obtain a \textit{non-degenerate} pairing $(-, -)_{\hat{\t}}$, let us declare that:
                    $$\d := (\bigoplus_{(r, s) \in \Z^2} \bbC D_{r, s}) \oplus \bbC D_v \oplus \bbC D_t$$
                wherein the basis vectors $D_{r, s}, D_v, D_t$ are, respectively, dual to the basis vectors $Z_{r, s}, c_v, c_t \in \z(\t)$ (as in remark \ref{remark: centres_of_dual_toroidal_lie_bialgebras}) with respect to $(-, -)_{\hat{\t}}$. Let us require also that:
                    $$(-, -)_{\hat{\t}}|_{\t} := (-, -)_{\t}$$
                as well as:
            ...
            \begin{remark}
                ...
                wherein:
                    ...
                    $$\z(\t_{[n]}) := \overline{\Omega^1_{[n]}} := \Omega^1_{A_{[n]}/\bbC}/d(A_{[n]})$$
                Recall also, from remark \ref{remark: centres_of_dual_toroidal_lie_bialgebras}, that the basis elements of $\z(\t_{[n]}) := \overline{\Omega^1_{[n]}}$ (with respect to the standard basis of $A_{[n]}$) are given by:
                    $$$$
                    
                Write $\d_{[n]} := \der_\bbC(A_{[n]})$ for the Lie algebra of $\bbC$-linear derivations on $A_{[n]}$, with the usual commutator bracket. Recall that as an $A_{[n]}$-module, $\d_{[n]}$ is free and of rank $n$ and is given by:
                    $$\d_{[n]} \cong \bigoplus_{1 \leq i \leq n} A_{[n]} \del_{v_i}$$
            \end{remark}
            \begin{convention}[Multi-indices] 
                In what follows, we will be using multi-indices. In particular, if $\vec{p} \in \Z^n$ and $\vec{v} := (v_1, ..., v_n)$ is an $n$-tuple (say, of formal variables) then:
                    $$\vec{v}^{\vec{p}} := v_1^{p_1} ... v_n^{p_n}$$
            \end{convention}
            \begin{definition}[Shifted full toroidal Lie algebras] \label{def: shifted_full_toroidal_lie_algebras}
                Now, we can consider the following \textbf{$\e$-shifted full $n$-toroidal Lie algebra}\footnote{We are interested in the case $n = 2$ and $\vec{\e} = (0, 1)$}, for some vector $\vec{\e} \in \Z_{\geq 0}^n$:
                    $$\hat{\t}_{[n]} := \t_{[n]} \oplus \d_{[n]}$$
                with the (adjoint) action of $\d_{[n]}$ on $\g \tensor_{\bbC} A_{[n]}$ being given by:
                    $$[ \vec{v}^{\vec{p}} \del_{v_i}, x \vec{v}^{\vec{q}} ]_{\hat{\t}_{[n]}} := q_i x \vec{v}^{\vec{p} + \vec{q} - \vec{\e}}$$
                for all $1 \leq i \leq n$, all $\vec{p} := (p_1, ..., p_n), \vec{q} := (q_1, ..., q_n) \in \Z^n$, and all $x \in \g$; this action extends (\textit{non-uniquely!}) to the universal central extension $\t_{[n]}$ in the following manner:
                    $$[ \vec{v}^{\vec{p}},  ]_{\hat{\t}_{[n]}}$$
            \end{definition}
            \begin{definition}[Shifted extended toroidal Lie algebras] \label{def: shifted_extended_toroidal_lie_algebras}

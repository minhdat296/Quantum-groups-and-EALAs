        ...
        \subsection{Extending \texorpdfstring{$ \uce(\g[v^{\pm}, t]) $}{} to fix degeneracy}
            ...
            \begin{remark}
                ...
                wherein:
                    ...
                    $$\z(\t_{[n]}) := \overline{\Omega_{[n]}^{\pm}} := \Omega_{[n]}^{\pm}/d(A_{[n]})$$
                with $\Omega_{[n]}^{\pm} := \Omega^1_{A_{[n]}/\bbC}$ and $d: A_{[n]} \to \Omega_{[n]}^{\pm}$ being the universal K\"ahler differential map; like before, we write $\overline{d}: A_{[n]} \to \overline{\Omega_{[n]}^{\pm}}$ for the composition of $d$ with the canonical $A_{[n]}$-module quotient map $\Omega_{[n]}^{\pm} \to \overline{\Omega_{[n]}^{\pm}}$.
                    
                Write $\d_{[n]}^{\pm} := \der_\bbC(A_{[n]}, A_{[n]})$ for the Lie algebra of $\bbC$-linear derivations on $A_{[n]}$, with the usual commutator bracket. Recall that as an $A_{[n]}$-module, $\d_{[n]}^{\pm}$ is free and of rank $n$, so let us pick for it the following $A_{[n]}$-linear basis:
            ...
            \begin{definition}[Shifted full $n$-toroidal Lie algebras] \label{def: shifted_full_toroidal_lie_algebras}
                We define the \textbf{$\e$-shifted full $n$-toroidal Lie algebra} associated to $\g$, for some fixed vector $\e \in \Z^n$, called the \textbf{degree shift}, to be the Lie algebra extension:
                    $$\widetilde{\t_{[n]}} := \t_{[n]} \rtimes \d_{[n]}^{\pm}$$
                with respect to the action of $\d_{[n]}^{\pm}$ on $\t_{[n]}$ being given by:
                    $$$$
            ...
            \end{convention}
            Let us now identify a basis for $\overline{\d_{[2]}^{\pm}}$.
            \begin{lemma}[A basis for $\overline{\d_{[2]}^{\pm}}$] \label{lemma: 2d_divergence_free_vector_fields_basis}
                ...
                is nothing but the Lie algebra of divergence-free vector fields on $\G_m \x \G_m$. The following basis can be identified for $\overline{\d_{[2]}^{\pm, \eta}}$:
            ...
            \end{lemma}
                ...
                \end{proof}
            \begin{proposition}[Pairing divergence-free vector fields with $1$-forms] \label{prop: pairing_2d_divergence_free_vector_fields_with_1_forms}
                Fix some $\eta \in \Z^2$.
            
                Consider $\overline{\Omega_{[2]}^{\pm}}$ as an abstract abelian Lie algebra and let $\d_{[2]}^{\pm}$ (and hence $\overline{\d_{[2]}^{\pm, \eta}}$) act on it as in definition \ref{def: shifted_full_toroidal_lie_algebras}. Then, on the semi-direct product:
                    $$\overline{\Omega_{[2]}^{\pm}} \rtimes \overline{\d_{[2]}^{\pm, \eta}}$$
                there will be a \textit{non-degenerate} invariant symmetric $\bbC$-bilinear form $\<-, -\>$ given by\footnote{With notations as in remarks \ref{remark: centres_of_dual_toroidal_lie_bialgebras} \ref{lemma: 2d_divergence_free_vector_fields_basis}}:
                    $$\< D^{\eta}_{p_1, p_2}, Z_{r, s} \> := \delta_{(p, q), (r, s)}$$
                    $$\< D^{\eta}_1, c_v \> := 1, \< D^{\eta}_2, c_t \> := 1$$
            ...
            \begin{corollary}
                ...
                Let $\d_{[2]}^{\pm}$ (and hence $\overline{\d_{[2]}^{\pm, \eta}}$) act on $\t_{[2]}$ as in definition \ref{def: shifted_full_toroidal_lie_algebras}. Then, there will be a \textit{non-degenerate} invariant symmetric $\bbC$-bilinear form $(-, -)_{\widehat{\t_{[2]}^{\eta}}}$ on:
                    $$\widehat{\t_{[2]}^{\eta}} := \t_{[2]} \rtimes \overline{\d_{[2]}^{\pm, \eta}} \cong \g_{[2]}^{\pm} \oplus ( \overline{\Omega_{[2]}^{\pm}} \oplus \overline{\d_{[2]}^{\pm, \eta}} )$$
                determined by:
                    $$(-, -)_{\widehat{\t_{[2]}^{\eta}}} |_{\Sym^2_{\bbC}( \g_{[2]}^{\pm} )} = (-, -)_{\g_{[2]}^{\pm}}$$
                    $$(-, -)_{\widehat{\t_{[2]}^{\eta}}} |_{\Sym^2_{\bbC}( \overline{\Omega_{[2]}^{\pm}} \oplus \overline{\d_{[2]}^{\pm, \eta}} )} = \<-, -\>$$
                    $$\left(\g_{[2]}^{\pm}, \overline{\Omega_{[2]}^{\pm}} \oplus \overline{\d_{[2]}^{\pm, \eta}} \right)_{\widehat{\t_{[2]}^{\eta}}} = 0, \left(\overline{\Omega_{[2]}^{\pm}}, \overline{\Omega_{[2]}^{\pm}}\right)_{\widehat{\t_{[2]}^{\eta}}} = 0, \left(\overline{\d_{[2]}^{\pm, \eta}}, \overline{\d_{[2]}^{\pm, \eta}}\right)_{\widehat{\t_{[2]}^{\eta}}} = 0$$
                with $(-, -)_{\g_{[2]}^{\pm}}$ being given by:
                    $$(x v^p, y v^q)_{\g_{[2]}^{\pm}} := (x, y)_{\g} \delta_{p + q, \e}$$
                for all $x, y \in \g$, $v := (v_1, v_2)$, and $p, q \in \Z^2$.
            \end{corollary}

\section{A root grading for Yangian extended toroidal Lie algebra}
    ...
    \subsection{Root space and triangular decompositions for Yangian extended toroidal Lie algebras}
        ...
        \begin{remark}[Toroidal root systems] \label{remark: toroidal_root_systems}
            The $\hat{Q}$-grading of $\extendedtoroidal$ as in theorem \ref{theorem: root_grading_for_extended_toroidal_lie_algebras} suggests to us that it is possible to construct a \say{toroidal root system}:
                $$\hat{\Phi}_{[2]}$$
            ...
            \begin{itemize}
                ...
                \item \todo[inline]{Is it closed under reflections ?}
            \end{itemize}
        ...
        \end{remark}
        \begin{definition}[Upper/lower toroidal roots] \label{def: upper_and_lower_toroidal_roots}
            The subset of $\hat{\Phi}_{[2]}$ consisting of \textbf{upper} elements is:
                $$\hat{\Phi}_{[2]}^{\up} := \hat{\Phi}_{[2]} \cap (\hat{Q}^+ \x \Z)$$
            and conversely, the \textbf{lower} elements form the subset:
                $$\hat{\Phi}_{[2]}^{\low} := \hat{\Phi}_{[2]} \cap (\hat{Q}^- \x \Z)$$
        \end{definition}
        ...
        \begin{proposition}[Triangular decomposition of extended toroidal Lie algebras] \label{prop: triangular_decomposition_of_extended_toroidal_lie_algebras}
            With respect to the choices of positive/negative roots as in remark \ref{remark: toroidal_root_systems}, let us set:
                $$\extendedtoroidal^{\up/\low} := \bigoplus_{\beta \in \hat{\Phi}_{[2]}^{\up/\low}} \extendedtoroidal_{\beta}$$
            The $\hat{Q}$-grading of the Lie algebra $\extendedtoroidal$ (respectively, of $\extendedtoroidal^{\positive/\negative}$) induces a triangular decomposition thereof as follows:
                $$\extendedtoroidal \cong \extendedtoroidal^{\low} \oplus \extendedtoroidal_0 \oplus \extendedtoroidal^{\up}$$
        \end{proposition}
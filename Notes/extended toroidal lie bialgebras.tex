...
\begin{document}
    ...
    \section{Extended toroidal Lie bialgebras as classical limits of affine Yangians}
        ...
        \subsection{Presentations for affine Yangians as associative algebras}
            \begin{definition}[Yangians associated to untwisted affine Kac-Moody algebras] \label{def: affine_yangians}
                (Cf. \cite[Definition 2.1]{guay_nakajima_wendlandt_affine_yangian_coproduct}) Let $\hbar$ be a formal variable. 
                
                The \textbf{formal Yangian} $\rmY_{\hbar}(\hat{\g}_{[1]}')$ associated to the \textit{derived subalgebra} $\hat{\g}_{[1]}'$ untwisted affine Kac-Moody algebra $\hat{\g}_{[1]}$ (as in convention \ref{conv: a_fixed_untwisted_affine_kac_moody_algebra}) is the associative $\bbC$-algebra generated by the set:
                    $$\{ H_{i, r}, E_{i, r}^{\pm} \}_{(i, r) \in \hat{\Gamma}_0 \x \N}$$
                whose elements are subjected to the following relations\footnote{Note the similarities with the Chevalley-Serre presentation of $\g$ in terms of the generating set $\{ h_i, e_i^{\pm} \}_{i \in \hat{\Gamma}_0}$.}, given for all $(i, r), (j, s) \in \hat{\Gamma}_0 \x \N$:
                    $$[ H_{i, r}, H_{j, s} ] = 0$$
                    $$[ E_{i, r}^+, E_{j, s}^- ] = \pm \delta_{ij} H_{i, r + s}$$
                    $$[ H_{i, 0}, E_{j, s}^{\pm} ] = \pm \hat{C}_{ij} E_{j, s}^{\pm}$$
                    $$[ H_{i, r + 1}, E_{j, s}^{\pm} ] - [ H_{i, r}, E_{j, s + 1}^{\pm} ] = \pm \frac12 \hbar \hat{C}_{ij} \{H_{i, r}, E_{j, s}^{\pm}\}$$
                    $$[ E_{i, r + 1}, E_{j, s}^{\pm} ] - [ E_{i, r}, E_{j, s + 1}^{\pm} ] = \pm \frac12 \hbar \hat{C}_{ij} \{E_{i, r}, E_{j, s}^{\pm}\}$$
                    $$\sum_{\sigma \in S_{N_{ij}}} \ad(E_{i, r_{\sigma(1)}}) \cdot ... \cdot \ad(E_{i, r_{\sigma(N_{ij})}}) \cdot E_{j, s}^{\pm} = 0, N_{ij} := 1 - \hat{A}_{ij}$$

                The \textbf{formal Yangian} $\rmY_{\hbar}(\hat{\g}_{[1]})$ associated to the untwisted affine Kac-Moody algebra $\hat{\g}_{[1]}$ itself is generated as an associative $\bbC$-algebra by the set:
                    $$\hat{\h}_{[1]} \cup \{ H_{i, r}, E_{i, r}^{\pm} \}_{(i, r) \in \hat{\Gamma}_0 \x \N}$$
                whose elements satisfy the same relations as those that define $\rmY_{\hbar}(\hat{\g}_{[1]}')$, as well as the following additional ones, given for all $i \in \hat{\Gamma}_0$ and all $h \in \h$:
                    $$H_{i, 0} := \frac12 \hat{C}_{ij} \hat{\alpha}_i^{\vee}$$
                    $$$$
        ...
        \subsection{Coproducts on affine Yangians}
            We review here the constructions of a coproduct and a quasi-coproduct giving rise, respectively, to a bialgebra and a quasi-bialgebra structure on the formal Yangian $\rmY_{\hbar}(\hat{\g}_{[1]})$. For more details, see \cite[Sections 4, 5, and 6]{guay_nakajima_wendlandt_affine_yangian_coproduct}. 
        ...
        \subsection{Classical limits of affine Yangians}
            \begin{theorem}[Extended toroidal Lie algebras as classical limits of affine Yangians] \label{theorem: extended_toroidal_lie_algebras_as_classical_limits_of_affine_yangians}
                The Lie bialgebra $\hat{\g}_{[2]}^+$ from corollarry \ref{coro: extended_toroidal_lie_bialgebras} is the classical limit\footnote{\say{Quasi-classical limit} in the terminology of \cite{etingof_kazhdan_quantisation_1}.} of the formal Yangian $\rmY_{\hbar}(\hat{\g}_{[1]})$ in the following sense:
                \begin{enumerate}
                    \item There is an isomorphisms of $\bbC$-bialgebras:
                        $$\rmY_{\hbar}(\hat{\g}_{[1]})/\hbar\rmY_{\hbar}(\hat{\g}_{[1]}) \cong \rmU(\hat{\g}_{[2]}^+)$$
                    \item One has that:
                        $$\frac{1}{\hbar}( \Delta_{\rmY_{\hbar}(\hat{\g}_{[1]})} - \Delta_{\rmY_{\hbar}(\hat{\g}_{[1]})}^{\cop} ) \equiv \delta_{\hat{\g}_{[2]}^+} \pmod{\hbar}$$
                \end{enumerate}
            \end{theorem}
                \begin{proof}

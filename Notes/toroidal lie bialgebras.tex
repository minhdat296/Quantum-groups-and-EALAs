...
\begin{document}
    ...
    \section{Constructing Lie cobrackets on (extended) toroidal Lie algebras}
        ...
        \begin{convention}[Yangians associated to symmetrisable Kac-Moody algebras]
            ...
            Recall also that if we let:
                $$\deg E_{i, r}^{\pm} = \deg H_{i, r} = r$$
                $$\deg \hbar = 1$$
            then in doing so, we will have endowed $\rmY_{\hbar}(\g)$ (respectively, $\rmY(\g)$) with a $\Z$-grading (respectively, a $\Z$-filtration); for each $k \in \Z$, let us write:
                $$\rmY_{\hbar}^k(\g), \rmY^k(\g)$$
            respectively to mean the $\bbC$-vector subspace consisting of degree-$k$ monomials. Furthermore - and even though this is not known to be true in general - we will also assume that there are \textbf{triangular decompositions}:
                $$\rmY_{\hbar}(\g) \cong \rmY_{\hbar}^{< 0}(\g) \tensor_{\bbC} \rmY_{\hbar}^0(\g) \tensor_{\bbC} \rmY_{\hbar}^{> 0}(\g)$$
                $$\rmY(\g) \cong \rmY^{< 0}(\g) \tensor_{\bbC} \rmY^0(\g) \tensor_{\bbC} \rmY^{> 0}(\g)$$
            wherein, for each $n \in \Z$, we have that:
                $$\rmY_{\hbar}^{\leq n}(\g) := \bigoplus_{k \in \Z_{\leq n}} \rmY_{\hbar}^k(\g)$$
                $$\rmY^{\leq n}(\g) := \bigcup_{k \in \Z_{\leq n}} \rmY^k(\g)$$
            (and likewise for $\geq$ and strict inequalities).
        \end{convention}
        \begin{convention}
        ...
        \subsection{Affine Yangians as quantisations of toroidal algebras}
            ...
            \end{lemma}
                \begin{proof}
                    ...
                    The assertion is now clear from remarks \ref{remark: total_degrees_of_classical_yangian_R_matrices} and \ref{remark: extended_toroidal_lie_bialgebras_explicit_formulae}.
                \end{proof}

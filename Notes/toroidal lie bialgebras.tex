            ...
            \begin{remark}[Lie cobracket on $\hat{\g}_{[2]}^+$, explicitly] \label{remark: extended_toroidal_lie_bialgebras_explicit_formulae}
                ...
                \begin{enumerate}
                    ...
                    \begin{enumerate}
                        ...
                        Together, these computations imply that:
                            $$[ x v^m t^p \tensor 1 + 1 \tensor x v^m t^p, \sfr_{ \z_{[2]}^+ }] = 0$$
                        \item Likewise, we have that (also for any $x \in \g$ and any $(m, p) \in \Z^2$):
                            $$
                                [ x v^m t^p \tensor 1 + 1 \tensor x v^m t^p, \sfr_{ \bar{\d}_{[2]}^+ }] = 
                                v^m t^p \left(
                                \begin{aligned}
                                ...
                                \right)
                            $$
                        For visual clarity, let us compute the summands individually; consider firstly the following:
                            $$
                                \begin{aligned}
                                    [ x t^{-1} \tensor 1 + 1 \tensor x t^{-1}, \del_v \tensor \bar{d}(v) ] & = [xt^{-1}, \del_v] \tensor \bar{d}(v) + \del_v \tensor [x t^{-1}, \bar{d}(v)]
                                    \\
                                    & = 0 \tensor \bar{d}(v) + \del_v \tensor 0
                                \end{aligned}
                            $$
                        and likewise, we see that:
                            $$[ x v^{-1} \tensor 1 + 1 \tensor x v^{-1}, \del_t \tensor \bar{d}(v) ] = 0$$
                        and so:
                            $$[ x v^m t^p \tensor 1 + 1 \tensor x v^m t^p, \sfr_{ \bar{\d}_{[2]}^+ }] = 0$$
                    \end{enumerate}
                    Note that the last two computations demonstrates that the auxiliary choice of a point $* \in \Z \x \Z_{> 0}$ is indeed inconsequential. 
                    \item Now, let us see how the elements $\delta_{\hat{\g}_{[2]}^+}(Z_{r, s})$ and $\delta_{\hat{\g}_{[2]}^+}(c_v)$ are given.
                    \begin{enumerate}

...
\begin{document}
    ...
    \section{Constructing Lie cobrackets on (extended) toroidal Lie algebras}
            ...
            \end{question}
                \begin{proof}
                    \begin{enumerate}
                        ...
                        By definition, the canonical element $\sfr_{\g[v_1^{\pm 1}, t]} \in \g[v_2^{\pm 1}, t_1] \hattensor_{\bbC} \g[v^{\pm 1}, t_2^{-1}]$ is given by:
                            $$\sfr_{\g[v_1^{\pm 1}, t]} := \sum_{(m, p) \in \Z^2} X_{m, p} \hattensor X_{m, p}^{\star}$$
        ...
        \begin{convention}
         ...
         \end{lemma}
            \begin{proof}
                \todo[inline]{This depends on PBW for untwisted affine Yangians. See appendix.}
            \end{proof}
    ...
    \section{Classical limits of affine Yangians}
        ...
        \subsection{Affine Yangians as quantisations of toroidal algebras}
            ...
            \end{theorem}
                \begin{proof}
                    We will demonstrate that the Lie bialgebra structure on $\hat{\g}_{[2]}^+$ as in corollary \ref{coro: extended_toroidal_lie_bialgebras} restricts down to the Lie subalgebra $\tilde{\g}_{[2]}^+$ thereof by showing that if:
                        $$X \in \{ E_{i, r}^{\pm}, H_{i, r} \}_{(i, r) \in \hat{\Gamma}_0 \x \Z_{\geq 0}}$$
                    is a Chevalley-Serre generator of $\tilde{\g}_{[2]}^+$ (cf. lemma \ref{lemma: chevalley_serre_presentation_for_central_extensions_of_multiloop_algebras}) then:
                        $$\delta_{\hat{\g}_{[2]}^+}(X) \in \tilde{\g}_{[2]}^+ \hattensor_{\bbC} \tilde{\g}_{[2]}^+$$
                    Actually, thanks to corollary \ref{coro: levendorskii_presentation__for_central_extensions_of_multiloop_algebras}, it suffices to only compute $\delta_{\hat{\g}_{[2]}^+}(X)$ for:
                        $$X \in \{ E_{i, r}^{\pm}, H_{i, r} \}_{(i, r) \in \hat{\Gamma}_0 \x \{0, 1\}}$$
                    Firstly, since:
                        $$\forall i \in \hat{\Gamma}_0: \deg E_{i, 0}^{\pm} = \deg H_{i, 0} = 0$$
                    it is clear (via remark \ref{remark: total_degrees_of_classical_yangian_R_matrices}) that:
                        $$\forall i \in \hat{\Gamma}_0: \delta_{\hat{\g}_{[2]}^+}(E_{i, 0}^{\pm}) = \delta_{\hat{\g}_{[2]}^+}(H_{i, 0}^{\pm}) = 0$$
                    To compute the action of the cocommutator $\delta_{\hat{\g}_{[2]}^+}$ on the degree-$1$ generators $E_{i, 1}^{\pm}, H_{i, 1}^{\pm}$, we shall be making use of the relations:
                        $$[ E_{i, r}^+, E_{j, s}^- ] = \delta_{ij} H_{i, r + s}$$
                        $$[ E_{i, 1}^{\pm}, E_{j, 0}^{\pm} ] - [ E_{i, 0}^{\pm}, E_{j, 1}^{\pm} ] = 0$$
                    (cf. corollary \ref{coro: levendorskii_presentation__for_central_extensions_of_multiloop_algebras}) along with the cocycle condition for Lie cobrackets of Lie bialgebras (cf. \cite[pp. 4]{etingof_kazhdan_quantisation_1}). Specifically, let us firstly consider the following:
                        $$\delta_{\hat{\g}_{[2]}^+}(H_{i, 1}) = \delta_{\hat{\g}_{[2]}^+}([ E_{i, 0}^+, E_{i, 1}^- ]) = [ \bar{\Delta}(E_{i, 0}^+), \delta_{\hat{\g}_{[2]}^+}(E_{i, 1}^-) ]$$
                    This tells us that (up to a sign correction), knowing $\delta_{\hat{\g}_{[2]}}(E_{i, 1}^{\pm})$ will help us completely determine $\delta_{\hat{\g}_{[2]}^+}(H_{i, 1})$; in particular, it remains to show that:
                        $$\delta_{\hat{\g}_{[2]}}(E_{i, 1}^{\pm}) \in \tilde{\g}_{[2]}^+ \hattensor_{\bbC} \tilde{\g}_{[2]}^+$$
                    To this end, we consider next the following:
                        $$0 = \delta_{\hat{\g}_{[2]}^+}( [ E_{i, 1}^{\pm}, E_{j, 0}^{\pm} ] - [ E_{i, 0}^{\pm}, E_{j, 1}^{\pm} ] ) = [ \delta_{\hat{\g}_{[2]}^+}(E_{i, 1}^{\pm}), \bar{\Delta}(E_{j, 0}^{\pm}) ] - [ \delta_{\hat{\g}_{[2]}^+}(E_{j, 1}^{\pm}), \bar{\Delta}(E_{i, 0}^{\pm}) ]$$
                \end{proof}

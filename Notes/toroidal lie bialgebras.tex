                    ...
                    \begin{enumerate}
                        ...
                        whenever $r \in \Z \setminus \{0\}$; in particular, the information that we wish to extract from this computation is that:
                            $$
                                ...
                                \end{aligned}
                            $$
                        Next, recall again from remark \ref{remark: centres_of_dual_toroidal_lie_bialgebras} that:
                        ...
                        and then consider the following:
                            $$
                                \begin{aligned}
                                    ...
                                    \\
                                    = & ( c_{t_1} \tensor h + h \tensor c_{t_2} ) \sum_{(r, s) \in (\Z \setminus \{0\}) \x \Z_{> 0}} \left( -v_1^r t_1^s v_2^{-r} t_2^{-s} + v_1^{-r} t_1^s v_2^r t_2^{-s} \right)
                                \end{aligned}
                            $$
                        ...
                        \item In order to compute $\hat{\delta}^+(t c_v)$, let us firstly use the fact that:
                            $$v^m t^p \bar{d}(v^n t^q) = \delta_{(m, p) + (n, q), (0, 0)} ( n c_v + q c_t ) + (np - mq) K_{m + n, p + q}$$
                        (cf. remark \ref{remark: centres_of_dual_toroidal_lie_bialgebras}) to see that:
                            $$t c_v = v^{-1} t c_v = K_{0, 1}$$
                        \begin{enumerate}
                            \item \textbf{($\g_{[2]}^+$-component):} Since $K_{0, 1} \in \z_{[2]}^+$ (which in particular means that it commutes with all elements of $\g_{[2]}$) and:
                                $$\sfr_{\g_{[2]}^+} = \sfr_{\g} v_2 \1(v_1, v_2) \1^+(t_1, t_2)$$
                            it is clear that:
                                $$[\bar{\Delta}(K_{0, 1}), \sfr_{\g_{[2]}^+}] = 0$$
                            \item \textbf{($\z_{[2]}^+$-component):} Recall from corollary \ref{coro: extended_toroidal_lie_bialgebras} that:
                                $$\sfr_{\z_{[2]}^+} := \sum_{(r, s) \in \Z \x \Z_{> 0}} K_{r, s} \tensor D_{r, s} + c_{v_1} \tensor D_{v_2}$$
                            and so:
                                $$
                                    \begin{aligned}
                                        & [\bar{\Delta}(K_{0, 1}), \sfr_{\z_{[2]}^+}] 
                                        \\
                                        = & \sum_{(r, s) \in \Z \x \Z_{> 0}} [\bar{\Delta}(K_{0, 1}), K_{r, s} \tensor D_{r, s}] + [\bar{\Delta}(K_{0, 1}), c_{v_1} \tensor D_{v_2}]
                                        \\
                                        = & \sum_{(r, s) \in \Z \x \Z_{> 0}} K_{r, s} \tensor [K_{0, 1}, D_{r, s}] + c_{v_1} \tensor [K_{0, 1}, D_{v_2}]
                                    \end{aligned}
                                $$
                            \item \textbf{($\d_{[2]}^+$-component):} Recall from corollary \ref{coro: extended_toroidal_lie_bialgebras} that:
                                $$\sfr_{\z_{[2]}^+} := \sum_{(r, s) \in \Z \x \Z_{\leq 0}} K_{r, s} \tensor D_{r, s} + c_{v_1} \tensor D_{v_2}$$
                            and so:
                                $$
                                    \begin{aligned}
                                        & [\bar{\Delta}(K_{0, 1}), \sfr_{\z_{[2]}^+}] 
                                        \\
                                        = & \sum_{(r, s) \in \Z \x \Z_{\leq 0}} [\bar{\Delta}(K_{0, 1}), D_{r, s} \tensor K_{r, s}] + [\bar{\Delta}(K_{0, 1}), D_{t_1} \tensor c_{v_2}]
                                        \\
                                        = & \sum_{(r, s) \in \Z \x \Z_{\leq 0}} [D_{r, s}, K_{0, 1}] \tensor K_{r, s} + [D_{t_1}, K_{0, 1}] \tensor c_{t_2}
                                    \end{aligned}
                                $$
                        \end{enumerate}
                    \end{enumerate}

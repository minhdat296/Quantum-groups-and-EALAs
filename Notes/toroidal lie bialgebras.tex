    ...
    \section{Classical limits of affine Yangians}
        ...
        \subsection{Toroidal Lie algebras as classical limits of affine Yangians}
            ...
            \begin{remark}[Lie cobracket on $\hat{\g}_{[2]}^+$, explicitly] \label{remark: extended_toroidal_lie_bialgebras_explicit_formulae}
                Let us see how the Lie cobracket $\delta_{\hat{\g}_{[2]}^+}$ from corollary \ref{coro: extended_toroidal_lie_bialgebras} is given explicitly on certain basis elements of $\hat{\g}_{[2]}^+$. These computations will be made use of in the proof of lemma \ref{lemma: toroidal_lie_bialgebras}; how the cobracket acts on elements of $\bar{\d}_{[2]}^+$ will not be of interest to us. 
                ...
                \begin{enumerate}
                    \item \textbf{(Action of $\hat{\delta}^+$ on $\g_{[2]}^+$):} Let us firstly see how the elements $\hat{\delta}^+(x v^m t^p)$, for some basis element $x \in \g$ and for some $(m, p) \in \Z \x \Z_{\geq 0}$, are given. Note that while it might seem that:
                        $$\hat{\delta}^+(x v^m t^p) = \delta_{\g_{[2]}^+}(x v^m t^p)$$
                    ...
                    \begin{itemize}
                        ...
                        To this end, consider firstly the following:
                            $$
                                \begin{aligned}
                                    & [Y_1 \tensor 1, \sfr_{\g_{[2]}^+}] 
                                    \\
                                    = & [Y_1 \tensor 1, \sfr_{\g} v_2 \1(v_1, v_2) \1^+(t_1, t_2)]
                                    \\
                                    = & [y_1 \tensor 1, \sfr_{\g}] v_2 \1(v_1, v_2) \1^+(t_1, t_2) - \sum_{1 \leq i \leq \dim_{\bbC} \g} \sum_{(m, p) \in \Z \x \Z_{\geq 0}} (y_1, x_i)_{\g} v_1^a t_1^b \bar{d}(v_1^m t_1^p) \tensor x_i^* v_2^a t_2^{b - 1}
                                \end{aligned}
                            $$
                        We know from remark \ref{remark: centres_of_dual_toroidal_lie_bialgebras} that:
                            $$v_1^a t_1^b \bar{d}(v_1^m t_1^p) = \delta_{(m, p) + (a, b), (0, 0)} ( a c_{v_1} + b c_{t_1} ) + (ap - mb) Z_{m + a, p + b}$$
                        and so:
                            $$
                        ...
                        Likewise, we have that:
                            $$
                                \begin{aligned}
                                    & [1 \tensor Y_2, \sfr_{\g_{[2]}^+}] 
                                    \\
                                    = & [Y_1 \tensor 1, \sfr_{\g} v_2 \1(v_1, v_2) \1^+(t_1, t_2)]
                                    \\
                                    = & [1 \tensor y_2, \sfr_{\g}] v_2 \1(v_1, v_2) \1^+(t_1, t_2) - \sum_{1 \leq i \leq \dim_{\bbC} \g} \sum_{(m, p) \in \Z \x \Z_{\geq 0}} x_i v_1^m t_1^p \tensor (x_i^*, y_2)_{\g} v_2^a t_2^b \bar{d}(v_2^{-m} t_2^{-p - 1})
                                    \\
                                    = & [1 \tensor y_2, \sfr_{\g}] v_2 \1(v_1, v_2) \1^+(t_1, t_2) - \sum_{1 \leq i \leq \dim_{\bbC} \g} x_i v_1^a t_1^{b - 1} \tensor (x_i^*, y_2)_{\g} (a c_{v_2} + b c_{t_2})
                                \end{aligned}
                            $$
                        ...
                        Putting the two computations together yields:
                            $$
                                [Y_1 \tensor 1 + 1 \tensor Y_2, \sfr_{\g_{[2]}^+}] =
                                \begin{aligned}
                                    & [y_1 \tensor 1 + 1 \tensor y_2] v_2 \1(v_1, v_2) \1^+(t_1, t_2)
                                    \\
                                    - & \sum_{1 \leq i \leq \dim_{\bbC} \g} \left( (y_1, x_i)_{\g} (a c_{v_1} + b c_{t_1}) \tensor x_i^* v_2^a t_2^{b - 1} + x_i v_1^a t_1^{b - 1} \tensor (x_i^*, y_2)_{\g} (a c_{v_2} + b c_{t_2}) \right)
                                \end{aligned}
                            $$
                    ...
                    Adding the three summands together yields:
                        $$
                            \begin{aligned}
                                & \hat{\delta}^+(Y)
                                \\
                                = & [Y_1 \tensor 1 + 1 \tensor Y_2, \sfr_{ \hat{\g}_{[2]}^+ }]
                                \\
                                = & 
                                \begin{aligned}
                                    & \left(
                    ...
                    \item \textbf{(Action of $\hat{\delta}^+$ on $\z_{[2]}^+$):} Next, we compute how $\hat{\delta}^+$ acts on the basis elements $Z_{r, s} \in \z_{[2]}^+$. 
                        ...
                        \item Secondly, consider the following:
                            $$
                                \begin{aligned}
                                    ...
                                    \\
                                    = & -\sum_{(a, b) \in \Z \x \Z_{> 0}} Z_{a, b} \tensor [D_{a, b}, Z_{r, s}] - c_{v_1} \tensor [D_{v_1}, Z_{r, s}]
                                \end{aligned}
                            $$
                            
                        \item Likewise, we have that:
                            $$[Z_{r, s} \tensor 1 + 1 \tensor Z_{r, s}, \sfr_{\bar{\d}_{[2]}^+}] = \sum_{(a, b) \in \Z \x \Z_{> 0}} [D_{a, b}, Z_{r, s}] \tensor Z_{a, b} + [D_{v_2}, Z_{r, s}] \tensor c_{v_2}$$
                            
                    \end{itemize}
                    Adding the three summands together then yields:
                        $$\hat{\delta}^+(Z_{r, s}) = \sum_{(a, b) \in \Z \x \Z_{> 0}} \left( -Z_{a, b} \tensor [D_{a, b}, Z_{r, s}] + [D_{a, b}, Z_{r, s}] \tensor Z_{a, b} \right) + \left( -c_{v_1} \tensor [D_{v_1}, Z_{r, s}] + [D_{v_2}, Z_{r, s}] \tensor c_{v_2} \right)$$
                \end{enumerate}
            \end{remark}
            ...
            \end{lemma}
                \begin{proof}
                    For convenience, let us abbreviate:
                        $$\hat{\delta}^+ := \delta_{\hat{\g}_{[2]}^+}$$
                    like in remark \ref{remark: extended_toroidal_lie_bialgebras_explicit_formulae}.
                    ...
                    for any $Y := y v^a t^b$, with $y \in \g$ and $(a, b) \in \Z \x \Z_{\geq 0}$. When $y := h_i$ for some \textit{arbitrary} vertex $i \in \hat{\Gamma}_0$ and $(a, b) = (0, 1)$, the above reduces to:
                        $$
                            \begin{aligned}
                                & \hat{\delta}^+(ht)
                                \\
                                = & [\bar{\Delta}(h), \sfr_{ \hat{\g}_{[2]}^+ }]
                                \\
                                = & \left(
                                \begin{aligned}
                                    & [\bar{\Delta}(h), \sfr_{\g}] v_2 \1(v_1, v_2) \1^+(t_1, t_2)
                                    \\
                                    - & \sum_{j \in \Gamma_0} \left( (h, h_j)_{\g} c_{t_1} \tensor h_j + h_j \tensor (h_j, h)_{\g} c_{t_2} \right)
                                \end{aligned}
                                \right) + \sum_{(r, s) \in \Z \x \Z_{> 0}} \left( Z_{r, s} \tensor y_2 D_{r, s}(t_2) + y_1 D_{r, s}(t_1) \tensor Z_{r, s} \right)
                                \\
                                = & \left(
                                \begin{aligned}
                                    & [\bar{\Delta}(h), \sfr_{\g}] v_2 \1(v_1, v_2) \1^+(t_1, t_2)
                                    ...
                                    - & \sum_{j \in \Gamma_0} \left( (h, h_j)_{\g} c_{t_1} \tensor h_j + h_j \tensor (h_j, h)_{\g} c_{t_2} \right)
                                \end{aligned}
                                \right) + \sum_{(r, s) \in \Z \x \Z_{> 0}} r \left( Z_{r, s} \tensor h v_2^{-r} t_2^{-s} + h v_1^{-r} t_1^{-s} \tensor Z_{r, s} \right)
                            \end{aligned}
                        $$
                    and we note that indeed, the total degree of $ht$ is $1$ while the total degree of $\hat{\delta}^+(ht)$ is $1 - 1 = 0$, agreeing with remark \ref{remark: total_degrees_of_classical_yangian_R_matrices}. 
                    ...
                    Because we have that:
                        $$[\bar{\d}_{[2]}^+, \z_{[2]}^+] \subseteq \z_{[2]}^+$$
                    (cf. remark \ref{remark: derivation_action_on_toroidal_centres}), it is clear form the formula for $\hat{\delta}^+(Z_{r, s})$ as above that:
                        $$\hat{\delta}^+(Z_{r, s}) \in \tilde{\g}_{[2]}^+ \hattensor_{\bbC} \tilde{\g}_{[2]}^+$$
                    which - in conjunction with how $\hat{\delta}^+(ht)$ is given for every $h \in \h$ - implies that the restriction:
                        $$\delta_{\tilde{\g}_{[2]}^+} := \hat{\delta}^+|_{\tilde{\g}_{[2]}^+}$$
                    indeed defines a Lie bialgebra structure on $\tilde{\g}_{[2]}^+$.
                    
                    Finally, let us specialise to the case $(r, s) = (0, 1)$ in order to explicitly compute $\hat{\delta}^+(Z_{0, 1})$. For this, it is enough to know how the brackets:
                        $$[D_{a, b}, Z_{0, 1}], [D_v, Z_{0, 1}]$$
                    are given explicitly. To this end, let us firstly recall from remark \ref{remark: centres_of_dual_toroidal_lie_bialgebras} that:
                    ...
                    which tells us that:
                        $$
                            \begin{aligned}
                                [D_{a, b}, Z_{0, 1}] & = [ -b v^{-a + 1} t^{-b - 1} \del_v + a v^{-a} t^{-b} \del_t, v^{-1} t \bar{d}(v) ]
                                \\
                                & = -b [ v^{-a + 1} t^{-b - 1} \del_v, v^{-1} t \bar{d}(v) ] + a [ v^{-a} t^{-b} \del_t, v^{-1} t \bar{d}(v) ]
                            \end{aligned}
                        $$
                    and that:
                        $$
                            \begin{aligned}
                                [D_v, Z_{0, 1}] & = [ v t^{-1} \del_v, v^{-1} t \bar{d}(v) ]
                                \\
                                & = 
                            \end{aligned}
                        $$

\section{Extended toroidal Lie (bi)algebras}
    ...
    \subsection{Extending \texorpdfstring{$\tilde{\g}_{[2]}$}{} to fix degeneracy} \label{subsection: extended_toroidal_lie_algebras}
        ...
        \begin{lemma}[How does $\d_{[2]}$ act on itself] \label{lemma: lie_bracket_on_orthogonal_complement_of_toroidal_centre}
        ...
        \end{lemma}
            \begin{proof}
                Pick arbitrary elements $D, D' \in \d_{[2]}$ and set:
                    $$[D, D']_{\hat{\g}_{[2]}} := X_{D, D'} + K_{D, D'} + \xi_{D, D'}$$
                for some $X_{D, D'} \in \g_{[2]}, K_{D, D'} \in \z_{[2]}$, and $\xi_{D, D'} \in \d_{[2]}$ depending on $D, D'$. Pick also a test element $y g(v, t) \in \g_{[2]}$, for some arbitrary $y \in \g$ and $g(v, t) \in A_{[2]}$ and set:
                    $$[D, y g(v, t)] := y D( g(v, t) ) + K_{D, Y}$$
                    $$[D', y g(v, t)] := y D'( g(v, t) ) + K_{D', Y}$$
                ...
                Via the Jacobi identity, we get that:
                    $$
                        \begin{aligned}
                            & 0
                            \\
                            = & [ y g(v, t), [D, D']_{\hat{\g}_{[2]}} ]_{\hat{\g}_{[2]}} + [ D, [ D', y g(v, t) ]_{\hat{\g}_{[2]}} ]_{\hat{\g}_{[2]}} + [ D', [ y g(v, t), D ]_{\hat{\g}_{[2]}} ]_{\hat{\g}_{[2]}}
                            \\
                            = & [ y g(v, t), [D, D']_{\hat{\g}_{[2]}} ]_{\hat{\g}_{[2]}} + [ D, y D'( g(v, t) ) + K_{D', Y} ]_{\hat{\g}_{[2]}} - [ D', y D( g(v, t) ) + K_{D, Y} ]_{\hat{\g}_{[2]}}
                        \end{aligned}
                    $$
                Now, consider:
                    $$[ D, y D'( g(v, t) ) + K_{D', Y} ]_{\hat{\g}_{[2]}} = y D( D'(g(v, t)) ) + [ D, K_{D', Y} ]_{\hat{\g}_{[2]}}$$
                    $$[ D', y D( g(v, t) ) + K_{D, Y} ]_{\hat{\g}_{[2]}} = y D'( D(g(v, t)) ) + [ D', K_{D, Y} ]_{\hat{\g}_{[2]}}$$
                and hence by subtracting the second expression from the first, we shall obtain:
                    $$
                        \begin{aligned}
                            & [ D, y D'( g(v, t) ) + K_{D, Y} ]_{\hat{\g}_{[2]}} - [ D', y D( g(v, t) ) + K_{D', Y} ]_{\hat{\g}_{[2]}}
                            \\
                            = & y (DD' - D'D)( g(v, t) ) + [ D, K_{D', Y} ]_{\hat{\g}_{[2]}} - [ D', K_{D, Y} ]_{\hat{\g}_{[2]}}
                        \end{aligned}
                    $$
                From this and from the computation with the Jacobi identity from before, we obtain:
                    $$[ [D, D']_{\hat{\g}_{[2]}}, y g(v, t) ]_{\hat{\g}_{[2]}} = y (DD' - D'D)( g(v, t) ) + [ D, K_{D', Y} ]_{\hat{\g}_{[2]}} - [ D', K_{D, Y} ]_{\hat{\g}_{[2]}}$$
                But at the same time, we have that:
                    $$
                        \begin{aligned}
                            & [ [D, D']_{\hat{\g}_{[2]}}, y g(v, t) ]_{\hat{\g}_{[2]}}
                            \\
                            = & [ X_{D, D'} + K_{D, D'} + \xi_{D, D'} , y g(v, t) ]_{\hat{\g}_{[2]}}
                            \\
                            = & [ X_{D, D'} + \xi_{D, D'} , y g(v, t) ]_{\hat{\g}_{[2]}}
                            \\
                            = & [ X_{D, D'} , y g(v, t) ]_{\hat{\g}_{[2]}} + \left( y \xi_{D, D'}(g(v, t)) + K_{\xi_{D, D'}, Y} \right)
                        \end{aligned}
                    $$
                wherein the second equality holds thanks to the fact that $[\z_{[2]}, \g_{[2]}]_{\hat{\g}_{[2]}} = 0$, and $K_{\xi_{D, D'}, Y} \in \z_{[2]}$ is some element (cf. remark \ref{remark: derivation_action_on_multiloop_algebras}). Putting everything together then yields:
                    $$
                        \begin{aligned}
                            & [ X_{D, D'} , y g(v, t) ]_{\hat{\g}_{[2]}} + \left( y \xi_{D, D'}(g(v, t)) + K_{\xi_{D, D'}, Y} \right)
                            \\
                        ...
                        \end{aligned}
                    $$
                From remark \ref{remark: centres_of_dual_toroidal_lie_algebras}, we know that there exists $K_{X_{D, D'}, Y} \in \z_{[2]}$ such that:
                    $$[ X_{D, D'} , y g(v, t) ]_{\hat{\g}_{[2]}} = [ X_{D, D'} , Y ]_{\hat{\g}_{[2]}} = [X_{D, D'}, Y]_{\g_{[2]}} + K_{X_{D, D'}, Y}$$
                using which we can write:
                    $$
                        \begin{aligned}
                            & [X_{D, D'}, Y]_{\g_{[2]}} + \left( K_{X_{D, D'}, Y} + K_{\xi_{D, D'}, Y} \right) - \left( [ D, K_{D', Y} ]_{\hat{\g}_{[2]}} - [ D', K_{D, Y} ]_{\hat{\g}_{[2]}} \right)
                            \\
                            = & y \left( ( DD' - D'D) - \xi_{D, D'} \right)( g(v, t) )
                        \end{aligned}
                    $$

                \textit{We now claim that:
                    $$\xi_{D, D'} = DD' - D'D$$
                i.e. that the $\d_{[2]}$-summand $\xi_{D, D'}$ of $[D, D']_{\hat{\g}_{[2]}}$ is nothing but the usual commutator of derivations}; should we be able to prove this fact, we would then have that:
                    $$[X_{D, D'}, Y]_{\g_{[2]}} + \left( K_{X_{D, D'}, Y} + K_{\xi_{D, D'}, Y} \right) - \left( [ D, K_{D', Y} ]_{\hat{\g}_{[2]}} - [ D', K_{D, Y} ]_{\hat{\g}_{[2]}} \right) = 0$$
                and since $[ D, K_{D', Y} ]_{\hat{\g}_{[2]}}, [ D', K_{D, Y} ]_{\hat{\g}_{[2]}} \in \z_{[2]}$ (cf. remark \ref{remark: derivation_action_on_toroidal_centres}) and $K_{X_{D, D'}, Y}, K_{\xi_{D, D'}, Y} \in \z_{[2]}$ by assumption, the above would then imply that:
                    $$[X_{D, D'}, Y]_{\g_{[2]}} \in \z_{[2]}$$
                as well, but this is the case if and only if:
                    $$[X_{D, D'}, Y]_{\g_{[2]}} = 0$$
                and because $Y \in \g_{[2]}$ was chosen arbitrarily and because $\g_{[2]}$ is centre-less\footnote{... meaning, in particular, that the only element of $\g_{[2]}$ commuting with every other element is $0$.} (as $\g$ is simple and the bracket on $\g_{[2]}$ is given by extension of scalars by assumption), this in turn implies that:
                    $$X_{D, D'} = 0$$
                i.e. that \textit{the $\g_{[2]}$-component of $[D, D']_{\hat{\g}_{[2]}}$ necessarily vanishes}.

                To prove the claim stated above, 
            \end{proof}
    ...
    \subsection{The centre $\hat{\z}_{[2]}$}
        \begin{question}
            What is the centre $\hat{\z}_{[2]} := \z( \hat{\g}_{[2]} )$ ? (this ought to be smaller than $\z_{[2]}$ somehow). 
        \end{question}
\section{Extended toroidal Lie (bi)algebras}
    ...
    \subsection{Extending \texorpdfstring{$\tilde{\g}_{[2]}$}{} to fix degeneracy} \label{subsection: extended_toroidal_lie_algebras}
        ...
        \begin{remark}[Lie bracket(s) on $\d_{[2]}$] \label{remark: dual_of_toroidal_centres_contains_derivations}
            ...
            \begin{enumerate}
                \item \textit{A priori}, one can suppose that:
                    $$[D, D']_{\hat{\g}_{[2]}} := X(D, D') + K(D, D') + \xi(D, D')$$
                for some $X(D, D') \in \g_{[2]}$, some $K(D, D') \in \z_{[2]}$, and some $\xi(D, D') \in \d_{[2]}$. 
                \begin{enumerate}
                    \item We claim that the $\g_{[2]}$-component $X(D, D')$ actually vanishes. To prove that this is the case, consider the following for any arbitrary $Y \in \g_{[2]}$, and without any loss of generality, we might as well take $Y := y g(v, t)$ for some $y \in \g$ and $g(v, t) \in A_{[2]}$:
                        $$
                            \begin{aligned}
                                ( [D, D']_{\hat{\g}_{[2]}}, Y )_{\hat{\g}_{[2]}} & = ( X(D, D') + K(D, D') + \xi(D, D'), Y )_{\hat{\g}_{[2]}}
                                \\
                                & = (X(D, D'), Y)_{\g_{[2]}}
                            \end{aligned}
                        $$
                    At the same time, we have by invariance that:
                        $$
                            \begin{aligned}
                                ( [D, D']_{\hat{\g}_{[2]}}, Y )_{\hat{\g}_{[2]}} & = ( D, [D', Y]_{\hat{\g}_{[2]}} )_{\hat{\g}_{[2]}}
                                \\
                                & = ( D, [D', Y]_{\hat{\g}_{[2]}} )_{\hat{\g}_{[2]}}
                                \\
                                & = ( D, y D( g(v, t) ) + K(Y) )_{\hat{\g}_{[2]}}
                                \\
                                & = ( D, K(Y) )_{\hat{\g}_{[2]}}
                            \end{aligned}
                        $$
                    wherein $K(Y)$ is some element\footnote{When $D = D_{r, s}$ and $g(v, t) = v^m t^p$, we have that $K(Y) = K_{(m, p), (r, s)}(y)$ as in \textit{loc. cit.}} of $\z_{[2]}$ depending on our choice of $y \in \g$ and $(m, p) \in \Z^2$ (cf. remark \ref{remark: derivation_action_on_multiloop_algebras}). We have thus that:
                        $$( X(D, D'), Y )_{\g_{[2]}} = ( D, K(Y) )_{\hat{\g}_{[2]}}$$
                    Suppose now that:
                        $$X(D, D') := \sum_{(m, p) \in \Z^2} x_{m, p} v^m t^p$$
                        $$Y := y v^n t^q$$
                    with the sum being understood to be finitely supported and for some $(n, q) \in \Z^2$, and also that:
                        $$D := \sum_{(a, b) \in \Z^2} \mu_{a, b} D_{a, b} + \mu_v D_v + \mu_t D_t$$
                        $$K(Y) := \sum_{(r, s) \in \Z^2} \lambda_{r, s} K_{r, s} + \lambda_v c_v + \lambda_t c_t$$
                    for some $\mu_{a, b}, \mu_v, \mu_t, \lambda_{r, s}, \lambda_v, \lambda_t \in \bbC$. We now have that:
                        $$\sum_{(m, p) \in \Z^2} (x_{m, p} y)_{\g} \delta_{(m, p) + (n, q), (0, -1)} = \sum_{(a, b) \in \Z^2} \sum_{(r, s) \in \Z^2} ( \mu_{a, b} D_{a, b} + \mu_v D_v + \mu_t D_t, \lambda_{r, s} K_{r, s} + \lambda_v c_v + \lambda_t c_t )_{\hat{\g}_{[2]}}$$
                    which gives:
                        $$(x_{-n, -q - 1}, y)_{\g} = \mu_{a, b} \lambda_{-a, -b} + \mu_v \lambda_v + \mu_t \lambda_t$$
                        
                    Let us now prove by contradiction that the summand $X(D, D') \in \g_{[2]}$ of $[D, D']_{\hat{\g}_{[2]}}$ actually vanishes. To this end, suppose for the sake of deriving a contradiction that:
                        $$\mu_{a, b} \lambda_{-a, -b} + \mu_v \lambda_v + \mu_t \lambda_t \not = 0$$
                    for all $(a, b) \in \Z^2$. This means that:
                        $$(x_{-n, -q - 1}, y)_{\g} \not = 0$$
                    for all $(n, q) \in \Z^2$ and all $x_{-n, -q - 1}, y \in \g$ as well. However, this can not be the case, e.g. when $x_{-n, -q - 1} \in \h$ and $y \in \n^{\pm}$, so we have a contradiction. The fact that:
                        $$\mu_{a, b} \lambda_{-a, -b} + \mu_v \lambda_v + \mu_t \lambda_t = 0$$
                    for all $(a, b) \in \Z^2$ now implies that:
                        $$( [D, D']_{\hat{\g}_{[2]}}, Y )_{\hat{\g}_{[2]}} = 0$$
                    for all $Y \in \g_{[2]}$, which in turn means that:
                        $$X(D, D') = 0$$
                    necessarily, seeing how $(-, Y)_{\hat{\g}_{[2]}}$ acts as zero on $\z_{[2]} \oplus \d_{[2]}$. 
                    ...
                    None of these expressions necessarily vanishes, so in general, neither does the central term $K(D, D')$. Thus we can only conclude that:
                        $$[D, D']_{\hat{\g}_{[2]}} \in \z_{[2]} \oplus \d_{[2]}$$
                    Ultimately, this does not matter too much when it comes to considering how $\d_{[2]}$ acts on $\tilde{\g}_{[2]}$, since $\z_{[2]}$ is the centre thereof and hence:
                        $$[ [\d_{[2]}, \d_{[2]}]_{\hat{\g}_{[2]}}, \tilde{\g}_{[2]} ]_{\hat{\g}_{[2]}} \subset [ \z_{[2]} \oplus \d_{[2]}, \tilde{\g}_{[2]} ]_{\hat{\g}_{[2]}} = [\d_{[2]}, \tilde{\g}_{[2]}]_{\hat{\g}_{[2]}}$$
                \end{enumerate}
                
                \item We now claim that, given how $\d_{[2]}$ acts on $\g_{[2]}$ (cf. remark \ref{remark: derivation_action_on_multiloop_algebras}), it can then be shown that the elements of $\d_{[2]}$ are in fact derivations on $A_{[2]}$. This is perhaps to be expected, since elements of $\z_{[2]}$ are certain equivalence classes of differential $1$-forms on $A_{[2]}$.
                \begin{enumerate}
                ...
                \item We can also identify the derivations $D_{r, s}, D_v, D_t$ explicitly in terms of $\del_v := \frac{\del}{\del v}, \del_t := \frac{\del}{\del t}$. For this, let us firstly equip $\der_{\bbC}(A_{[2]})$ - the $\bbC$-vector space of all $\bbC$-linear derivations on $A_{[2]}$ - with the following basis:
        ...
        \end{theorem}
        \begin{definition}[Extended toroidal Lie algebras] \label{def: extended_toroidal_lie_algebras}
        ...
        \end{definition}
        \begin{remark}
            Our notion of extended $2$-toroidal Lie algebras does not quite coincide with the very similar notion of an \say{extended affine Lie algebra of nullity $2$}. Ultimately, this is because the bilinear form that we have endowed $\g_{[2]}$ with is of degree $-1$ (as opposed to $0$) in the second variable. For the latter, $\d_{[2]}$ would be the Lie algebra of divergence-free vector fields on $\Spec A_{[2]}$.
        \end{remark}

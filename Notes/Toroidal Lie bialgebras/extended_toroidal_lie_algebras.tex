\section{Extended toroidal Lie (bi)algebras}
    ...
    \subsection{Extending \texorpdfstring{$\tilde{\g}_{[2]}$}{} to fix degeneracy} \label{subsection: extended_toroidal_lie_algebras}
        ...
        \begin{remark}[Computing the centre without computing all the brackets ...]
            ...
            for any $X, Y \in \hat{\g}_{[2]}$, with the first equality holding thanks to the fact that $Z$ is supposed to commute with every other element of $\hat{\g}_{[2]}$ by assumption of being central. Per the construction of $(-, -)_{\hat{\g}_{[2]}}$, we have that:
                $$(\z_{[2]} \oplus \d_{[2]}, \g_{[2]})_{\hat{\g}_{[2]}} = 0$$
                $$\z_{[2]}^{\bot} = \d_{[2]}$$
            At the same time, we also know that:
            \begin{itemize}
                \item $[\g_{[2]}, \g_{[2]}]_{\hat{\g}_{[2]}} \subset \g_{[2]} \oplus \z_{[2]}$ which is by construction,
                \item $[\d_{[2]}, \z_{[2]}]_{\hat{\g}_{[2]}} \subseteq \z_{[2]}$ (cf. remark \ref{remark: derivation_action_on_toroidal_centres}),
                \item $[\d_{[2]}, \d_{[2]}]_{\hat{\g}_{[2]}} \subseteq \d_{[2]} \oplus \z_{[2]}$ (cf. proposition \ref{prop: lie_bracket_on_orthogonal_complement_of_toroidal_centre}). 
            \end{itemize}
            Therefore, we can make the conclusion that $\hat{\z}_{[2]}$ must only be the span of certain basis elements of $\z_{[2]}$. 
        \end{remark}
        \begin{proposition}
            The centre $\hat{\z}_{[2]}$ is two-dimensional, namely spanned by $c_v$ and $c_t$. 
        \end{proposition}
            \begin{proof}
                Since $c_v, c_t$ are orthogonal to $D_v, D_t$ respectively and parallel to $D_{r, s}$ with respect to $(-, -)_{\hat{\g}_{[2]}}$, for all $(r, s) \in \Z^2$, per the construction of $(-, -)_{\hat{\g}_{[2]}}$, it suffices to show that:
                    $$[D_v, D_t]_{\hat{\g}_{[2]}} \in \bbC D_v \oplus \bbC D_t$$
                but does not lie within either direct summand alone. This can be done easily by explicit computation, for which we simply need to know that the $\d_{[2]}$-summands of the bracket of elements of $\d_{[2]}$ are nothing but the commutator brackets of derivations (cf. proposition \ref{prop: lie_bracket_on_orthogonal_complement_of_toroidal_centre}). 
            \end{proof}
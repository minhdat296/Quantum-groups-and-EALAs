\section{Quivers, hereditary rings, and homological dimension \texorpdfstring{$\leq 1$}{}}
    \subsection{Quivers}
        ...
        \subsubsection{(Co)reflection functors and Gabriel's Theorem}
            ...
            \begin{convention}
                From now on, we shall be working over a commutative ring $k$.
            \end{convention}
                ...
                Let $Q := (Q_1, Q_0, s, t)$ be a quiver and $\sigma \in Q_0$ be a fixed vertex. 
                    \begin{itemize}
                        \item If $v$ is a sink, then one can define the so-called \textbf{reflection functor}:
                            $$\Refl_{\sigma}: \Rep_k(Q) \to \Rep_k(Q)$$
                            $$(\calF: [Q] \to k\mod) \mapsto \left( \Refl_{\sigma}(\calF): [Q] \to k\mod: v \mapsto \begin{cases} \text{$\calF(v)$ if $v \in Q_0 \setminus \{\sigma\}$} \\ \text{$\underset{v \in [Q]_{/\sigma}}{\lim} \calF(v)$ if $v = \sigma$} \end{cases} \right)$$
                        \item If $v$ is a cosink, then one can instead define the \textbf{coreflection functor}:
                            $$\co\Refl_{\sigma}: \Rep_k(Q) \to \Rep_k(Q)$$
                            $$(\calF: [Q] \to k\mod) \mapsto \left( \Refl_{\sigma}(\calF): [Q] \to k\mod: v \mapsto \begin{cases} \text{$\calF(v)$ if $v \in Q_0 \setminus \{\sigma\}$} \\ \text{$\underset{v \in {}^{\sigma/}[Q]}{\colim} \calF(v)$ if $v = \sigma$} \end{cases} \right)$$
                    \end{itemize}
            \end{definition}
            \begin{convention}
                Let $Q := (Q_1, Q_0, s, t)$ be a quiver and $\sigma \in Q_0$ be a fixed vertex. Then, for all $\calF \in \Ob(\Rep_k(Q))$, we shall abuse notations slightly and write:
                    $$\Refl_{\sigma}(\calF) := \underset{v \in [Q]_{/\sigma}}{\lim} \calF(v)$$
                    $$\co\Refl_{\sigma}(\calF) := \underset{v \in {}^{\sigma/}[Q]}{\colim} \calF(v)$$
            ...
            \end{convention}
            \begin{remark}[Categorical properties of (co)reflection functors]
                ...
                is a diagram of $k$-linear representations of $Q$, then clearly:
                    $$\Refl_{\sigma}\left( \underset{i \in I^{\op}}{\lim} \calF_i \right) \cong \underset{i \in I}{\lim} \Refl_{\sigma}(\calF)$$
                and:
                    $$\co\Refl_{\sigma}\left( \underset{i \in I}{\colim} F_i \right) \cong \underset{i \in I}{\colim} \co\Refl_{\sigma}(F)$$
            ...
            \end{remark}
            \begin{remark}[Successive compositions of (co)reflection functors and the directionality of Dynkin quivers] \label{remark: dynkin_quivers_are_unique_up_to_orientations}
                ...
                one can act upon it with the following composition of reflection and coreflection functors (or indeed, any permutation thereof):
                    $$\co\Refl_{\sigma_n} \circ \Refl_{\sigma_n} \circ ... \circ \co\Refl_{\sigma_1} \circ \Refl_{\sigma_1}: \Rep_k(Q) \to \Rep_k(Q)$$
                in order to obtain a filtered diagram of $k$-modules:
                    $$\left(\co\Refl_{\sigma_n} \circ \Refl_{\sigma_n} \circ ... \circ \co\Refl_{\sigma_1} \circ \Refl_{\sigma_1}\right)(\calF): [Q] \to k\mod$$
                From this, one infers that the representation theory of Dynkin quivers would actually be \textit{independent} of their orientations, in the sense that the category of representations $\Rep_k(Q)$ of a Dynkin quiver $Q := (Q_1, Q_0, s, t)$ is unique up to different direction of the directed edges $x \in Q_1$, \textit{should we be able to show that the (co)reflection functors are auto-equivalences of $\Rep_k(Q)$}.
            \end{remark}
            \begin{remark}[(Co)reflection functors of finite quivers are exact] \label{remark: (co)reflection_functors_of_finite_quivers_are_exact}
                ...
                \begin{enumerate}
                    ...
                    is a \textit{finite} quiver with a fixed vertex $\sigma \in Q_0$ then the corresponding (co)reflection functors:
                        $$\Refl_{\sigma}, \co\Refl_{\sigma}: \Rep_k(Q) \to \Rep_k(Q)$$
                    ...
                    with any permutation of the composition:
                        $$\co\Refl_{\sigma_n} \circ \Refl_{\sigma_n} \circ ... \circ \co\Refl_{\sigma_1} \circ \Refl_{\sigma_1}$$
                    we would obtain a filtered diagram of $k$-modules:
                        $$(\co\Refl_{\sigma_n} \circ \Refl_{\sigma_n} \circ ... \circ \co\Refl_{\sigma_1} \circ \Refl_{\sigma_1})(\calF): [\Gamma] \to k\mod$$
                    and therefore we might as well assume that Dynkin quivers are filtered. Since Dynkin quivers are finite, we thus gather from the discussion in the previous paragraph that in fact, the (co)reflection functors:
                        $$\Refl_{\sigma}, \co\Refl_{\sigma}: \Rep_k(\Gamma) \to \Rep_k(\Gamma)$$
                    are \textit{exact} and will preserve simple objects (i.e. irreducibility $k$-linear representations of $\Gamma$). As such, if $\Gamma := (\Gamma_1, \Gamma_0, s, t)$ is a Dynkin quiver and $\sigma \in \Gamma_0$ is a fixed vertex, then the coreflection functors $\Refl_{\sigma}$ and $\co\Refl_{\sigma}$ will admit restrictions:
                        $$\Refl_{\sigma}^{\irr} := \Refl_{\sigma}|_{\Rep_k^{\irr}(\Gamma)}$$
                        $$\co\Refl_{\sigma}^{\irr} := \co\Refl_{\sigma}|_{\Rep_k^{\irr}(\Gamma)}$$
                    down onto the full subcategory $\Rep_k^{\irr}(\Gamma) \subseteq \Rep_k(\Gamma)$ spanned by the irreducible $k$-linear representations of $Q$. Since morphisms between irreducible representations (i.e. simple objects of $\Rep_k(\Gamma)$) are known to either be zero or invertible by Schur's Lemma (cf. lemma \ref{lemma: schur_lemma_for_abelian_categories}), \textit{the restricted (co)reflection functors $\Refl_{\sigma}^{\irr}$ and $\co\Refl_{\sigma}^{\irr}$ are also exact.}
                \end{enumerate}
            \end{remark}
            
            Let us now recall the following classical result from the representation theory of finite-dimensional algebras over fields.  
            \begin{lemma}[Projective indecomposable modules over finite algebras are simple] \label{lemma: projective_indecomposable_modules_over_finite_algebras_are_simple}
                \cite[Theorem 7.1]{leinster_projective_indecomposable_modules_are_simple}\footnote{One interesting consequence of this lemma (which is not too relevant to the current discussion about Dynkin quivers, as path algebras of quivers are generally not semi-simple, only hereditary) is that it implies via lemma \ref{lemma: projective_and_injective_modules_over_semi_simple_rings} that over a semi-simple ring, a module is indecomposable if and only if it is simple.} Let $k$ be a field and $A$ be a finite-dimensional $k$-algebra. Then, there exists a bijection:
                    $$
                        \left\{\text{simple left/right-$A$-modules}\right\}
                        \to
                        \left\{\text{projective indecomposable left/right-$A$-modules}\right\}
                    $$
                sending each simple left/right-$A$-module $M$ to a choice of projective cover $\e: P \to M$.
            \end{lemma}
                \begin{proof}
                    For details, we refer the reader to \cite[Propositions 5.5 and 6.2]{leinster_projective_indecomposable_modules_are_simple}, but let us nevertheless recall the steps involved in the proof of this result.
                    
                    First of all, in order to show that for each simple left/right-$A$-module, there exists some , start by recalling that simple left/right-modules are cyclic\footnote{The proof is simple (pun not intended!): if we were to suppose to the contrary that there existed a simple module $M$ with $\geq 1$ generators, then said module will admit non-zero (cyclic) proper submodules generated by the generators, and therefore $M$ can not be simple, contradicting our initial assumption.}. This tells us that every simple left/right-$A$-module admits a projective cover by a free\footnote{We assume the Axiom of Choice, so that free module would be projective.} left/right-$A$-module on $1$ generator, which is of course unique up to isomorphisms, per the universal property of left-adjoints.
                    
                    On the other hand, 
                \end{proof}
            \begin{corollary}
                Let $k$ be a field and $Q$ be a finite quiver. Then, a $k$-linear representation of $Q$ is irreducible if and only if it is indecomposable and projective.
            \end{corollary}
            \begin{lemma}[Filtered colimits of simple objects are indecomposable] \label{lemma: filtered_colimits_of_simple_objects_are_indecomposable}
                Let $\calA$ be an abelian category. Then, the ind-completion\footnote{Let us ignore set-theoretic issues here.} of the full-subcategory $\calA^{\simple}$ of simple objects is $\calA^{\indecomp}$, the full subcategory of indecomposable objects:
                    $$\calA^{\indecomp} \cong \Ind(\calA^{\simple})$$
            \end{lemma}
                \begin{proof}
                    
                \end{proof}
            \begin{corollary}[(Co)reflections of indecomposable representations of quivers] \label{coro: (co)reflections_of_indecomposable_representations_of_quivers}
                Let $\Gamma := (\Gamma_1, \Gamma_0, s, t)$ be a Dynkin quiver\footnote{So that the (co)reflection functors will be exact and therefore will preserve irreducibility of representations (cf. remark \ref{remark: (co)reflection_functors_of_finite_quivers_are_exact}).} with a fixed vertex $\sigma \in \Gamma_0$ and let $k$ be a commutative ring. Then:
                    $$\Rep_k^{\red}(\Gamma) \cong \Ind( \Rep_k^{\irr}(\Gamma) )$$
                which gives rise to restrictions:
                    $$\Refl_{\sigma}^{\red} := \Ind(\Refl_{\sigma}^{\irr}) = \Refl_{\sigma}|_{\Rep_k^{\red}(\Gamma)}$$
                    $$\co\Refl_{\sigma}^{\red} := \Ind(\co\Refl_{\sigma}^{\irr}) = \co\Refl_{\sigma}|_{\Rep_k^{\red}(\Gamma)}$$
                of the (co)reflection functors $\Refl_{\sigma}$ and $\co\Refl_{\sigma}$ based at some fixed vertex $\sigma \in \Gamma_0$ down onto the full subcategory $\Rep_k^{\red}(\Gamma)$ of indecomposable $k$-linear representations of $\Gamma$. 
                
                In particular, this means that should:
                    $$\calF \in \Ob(\Rep_k^{\red}(\Gamma))$$
                be any indecomposable $k$-linear representations of $Q$ then $\Refl_{\sigma}(\calF)$ and $\co\Refl_{\sigma}(\calF)$ will either be zero (which also means that it is indecomposable according to definition \ref{def: (in)decomposable_objects}) or, in the event that it is non-zero, also indecomposable.
            \end{corollary}
                \begin{proof}
                    The first part is self-evident. For the second, simply apply Schur's Lemma (cf. lemma \ref{lemma: schur_lemma_for_abelian_categories}) followed by lemma \ref{lemma: filtered_colimits_of_simple_objects_are_indecomposable}.
                \end{proof}
            \begin{theorem}[Gabriel's theorem on Dynkin quivers] \label{theorem: gabriel_theorem_on_dynkin_quivers}
                Let $k$ be a field and $Q$ be a quiver. $Q$ is then a Dynkin quiver if and only if $\Rep_k(\Gamma)$ has finitely many isomorphism classes of indecomposable objects.
            \end{theorem}
                \begin{proof}
                    
                \end{proof}
            \begin{corollary}[Dimension vectors of indecomposable representations of Dynkin quivers] \label{coro: dimension_vectors_of_indecomposable_representations_of_dynkin_quivers}

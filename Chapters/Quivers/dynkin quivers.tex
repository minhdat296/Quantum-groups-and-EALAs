    ...
    \subsection{Quivers}
        ...
        \subsubsection{Roots, Grothendieck groups, and Euler characteristics of Dynkin quivers}
            ...
            \begin{definition}[Simply laced quivers] \label{def: simply_laced_quivers}
                A quiver $Q := (Q_1, Q_0, s, t)$ is \textbf{simply laced} if and only if none of its vertices has self-loops: that is, for all $v \in Q_0$, one has:
                    $$\{f \in Q_1 \mid s(f) = t(f) = v\} = \{\id_v\}$$
                Equivalently, one says that a quiver $Q$ is simply-laced if and only if for all $v \in \Ob([Q])$, one has:
                    $$[Q](v, v) = \{\id_v\}$$
            \end{definition}
            \begin{remark}
            ...
            \begin{remark}[Taking dimension vectors is $\Z$-linear] \label{remark: taking_dimension_vectors_is_Z_linear}
                ...
                for all $\calF, \calF' \in \Ob(\Rep_k(Q))$. From this, one sees that there is an induced $\Z$-module homomorphism:
            ...
            \end{proposition}
                \begin{proof}
                    This comes from the fact that the simple roots of $\Gamma$ form a basis for $\Z^{\oplus \Gamma_0}$ (cf. example \ref{example: simple_roots}), which we know by lemma \ref{lemma: representations_of_dynkin_quivers_are_determined_by_dimension_vectors} is isomorphic to the Grothendieck group $\K_0(\Gamma)$ via:
            ...
            \begin{example}[Euler characteristics of quiver representations] \label{example: euler_characteristics_of_quiver_representations}
                Let $Q$ be a finite quiver and $k$ be a field. The Euler characteristic of $Q$ is then defined to be the Euler characteristic of the right-derived functor (which is indeed triangulated since the underived functor $\Hom_{k\<Q\>}(-, \calF)$ is left-exact \textit{a priori}):
                    $$\R\Hom_{k\<Q\>}(\calF, -): \rmD^b(\Rep_k(Q)) \to \rmD^b(k\-\Vect^{\fin})$$
                evaluated at $\calF$ itself (or rather, some projective resolution $\calF^{\bullet}$ thereof), i.e.:
                    $$
                        \begin{aligned}
                            \chi_Q(\calF^{\bullet}) & := \chi(\R\Hom_{k\<Q\>}(-, \calF), \calF^{\bullet})
                            \\
                            & = \sum_{i \in \Z} (-1)^i \dim_k \R\Hom_{k\<Q\>}^i(\calF^{\bullet}, \calF)
                            \\
                            & = \sum_{i \in \Z} (-1)^i \dim_k \Ext^i_{k\<Q\>}(\calF, \calF)
                        \end{aligned}
                    $$
                
                Now, if $A$ is a hereditary $k$-algebra (e.g. $A \cong k\<Q\>$), then one has by definition that:
                    $$\forall i \geq 2: \Ext^i_A(M, N) \cong 0$$
                for all left/right-$A$-modules $M, N$, so in fact, one has:
                    $$\chi_Q(\calF) = \dim_k \End_{k\<Q\>}(\calF) - \dim_k \Ext^1_{k\<Q\>}(\calF, \calF)$$
                for all representations $\calF \in \Ob(\Rep_k(Q))$. Since we will be working over a field for the remainder of this subsection, and since we are only interested in Dynkin quivers which are finite by definition, we might as well take as a \textit{definition} that the Euler characteristic of a finite quiver $Q$ is given by:
                    $$\chi_Q: \Ob(\rmD^b(\Rep_k(Q))) \to \Z$$
                    $$\calF \mapsto \dim_k \End_{k\<Q\>}(\calF) - \dim_k \Ext^1_{k\<Q\>}(\calF, \calF)$$
            \end{example}
            ...
            \end{lemma}
                \begin{proof}
                    Straightforward on the fact that the cohomological functor $F: \rmD^b(\E)^{\op} \to \rmD^b(k\mod^{\fin})$ is triangulated by definition (cf. convention \ref{conv: cohomological_functors_dynkin_quiver_representations}) and from the definition of Grothendieck groups (cf. definitions \ref{def: simple_grothendieck_groups} and \ref{def: projective_grothendieck_groups}).
                \end{proof}
            \begin{theorem}[Tits quadratic forms as Euler characteristics] \label{theorem: tits_quadratic_forms_as_euler_characteristics}
                Let $k$ be a field and let $\Gamma := (\Gamma_1, \Gamma_0, s, t)$ be a Dynkin quiver, and let us denote the Cartan quadratic form of this quiver by $B_{\Gamma}$ (cf. definition \ref{def: cartan_quadratic_forms_of_finite_quivers}). Then, there is a commutative diagram of $\Z$-module and homomorphisms between them as follows:
                    $$
                        \begin{tikzcd}
                        	{\K_0(\Gamma)} & {\Z^{\oplus \Gamma_0}} \\
                        	& \Z
            ...
            \end{theorem}
                \begin{proof}
                    Since $\K_0(\Gamma)$ is spanned by the irreducible $k$-linear representations of $\Gamma$ (cf. proposition \ref{prop: simple_grothendieck_groups_of_finite_linear_abelian_categories_are_free_on_simple_objects}), it shall suffice to only prove that:
                        $$\chi_{\Gamma}(\calF) = q_{\Gamma}(\dim_k \calF)$$
                    for all $\calF \in [\Rep_k^{\irr}(\Gamma)]$; to this end, we claim that:
                        $$\dim_k \End_{k\<\Gamma\>}(\calF) = \sum_{1 \leq i \leq n} (\dim_k \calF(v_i))^2$$
                    and:
                        $$\dim_k \Ext^1_{k\<\Gamma\>}(\calF, \calF) = \sum_{1 \leq i \leq j \leq n} \dim_k \calF(v_i) r_{ij} \dim_k \calF(v_j)$$
                    for all $\calF \in [\Rep_k^{\irr}(\Gamma)]$ and some fixed enumeration $\Gamma_0 := \{v_1, ..., v_n\}$ and here, $R_{\Gamma} := (r_{ij})_{1 \leq i, j \leq n}$ is the adjacency matrix of $\Gamma$; here, our convention is that $i \leq j$ whenever $[\Gamma](v_i, v_j) \not = \varnothing$ (i.e. if there is an arrow from $v_i$ to $v_j$). To show that the first statement holds, simply observe that:
                        $$
                            \begin{aligned}
                                \dim_k \End_{k\<\Gamma\>}(\calF) & = \sum_{1 \leq i \leq n} \dim_k \End_k(\calF(v_i))
                                \\
                                & = \sum_{1 \leq i \leq n} (\dim_k \calF(v_i))^2
                            \end{aligned}
                        $$
                    and in order to prove the second statement, note that:
                        $$
                            \begin{aligned}
                                \dim_k \Ext^1_{k\<\Gamma\>}(\calF, \calF) & = \sum_{1 \leq i \leq j \leq n} \dim_k \Ext^1_k(\calF(v_i), \calF(v_j))
                                \\
                                & = \sum_{1 \leq i \leq j \leq n} 
                                \\
                                & = \sum_{1 \leq i \leq j \leq n} \dim_k \calF(v_i) r_{ij} \dim_k \calF(v_j)
                            \end{aligned}
                        $$
                    wherein the last equality comes from the definition of the adjacency matrix (cf. definition \ref{def: cartan_quadratic_forms_of_finite_quivers}).
                \end{proof}
        ...
        \subsubsection{(Co)reflection functors and Gabriel's Theorem}
            ...
            in order to show that there is bijection between the isomorphism classes of indecomposable $k$-linear representations of $\Gamma$ and the positive roots thereof. 
            ...
            \end{convention}
            \begin{convention}
                From now on, let us work over a field $k$ so that path $k$-algebras of finite quivers are hereditary. 
            \end{convention}

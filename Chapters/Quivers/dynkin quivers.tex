        ...
        \subsubsection{Generalities about quivers and their representations}
            ...
            \begin{example}[Path algebras of discrete quivers]
                ...
                It is easy to see that the path algebra of this quiver (over some fixed ring $k$) is:
                    $$k\<e\>/\<e^2 = e\> \cong k$$
                (the isomorphism with $k$ as associative $k$-algebras is due to the fact that by construction, $k\<e\>/\<e^2 = e\>$ is of rank $1$ as a $k$-module), with one small note on the side being that the generator $e$ is not \textit{a priori} invertible, since formally, there does not exist a morphism $\id^{-1}$ in the free category generated by the given quiver, and one is only permitted to compose \say{forwardly}, i.e. to consider the compositions $\id, \id^2, \id^3, ...$. 

    ...
    \subsection{Quivers}
        ...
        \subsubsection{(Co)reflection functors and admissible enumerations of vertices}
            ...
            \end{proposition}
                ...
                \end{proof}
            \begin{lemma}[(Co)reflection functors on finite connected acyclic quivers are adjoint] \label{lemma: (co)reflection_functors_on_finite_connected_acyclic_quivers_are_adjoint}
            ...
            \begin{convention}[Kronecker representations] \label{conv: kronecker_representations}
                Let $\Gamma := (\Gamma_1, \Gamma_0, s, t)$ be a Dynkin quiver and enumerate its vertices by:
                    $$\Gamma := \{\sigma_1, ..., \sigma_n\}$$
                where $n := |\Gamma_0|$. Then for all $1 \leq i, j \leq n$, the $i^{th}$ \textbf{Kronecker representation} of $\Gamma$ is then the object $\Delta_i \in \Ob(\Rep_k^{\irr}(\Gamma))$ given by:
                    $$\Delta_i(\sigma_j) := k^{\oplus \delta_{ij}}$$
            \end{convention}
            ...
            \end{lemma}
                ...
                \end{proof}
            \begin{lemma}[(Co)reflections of indecomposable representations] \label{lemma: (co)reflections_of_indecomposable_representations}
                Let $\Gamma := (\Gamma_1, \Gamma_0, s, t)$ be a Dynkin quiver and fix an arbitrary vertex $\sigma \in \Gamma_0$. Then, for all representations $\calF \in \Ob(\Rep_k(\Gamma))$, the (co)reflections:
                    $$\Refl_{\Gamma, \sigma}(\calF), \co\Refl_{\Gamma, \sigma}(\calF)$$
                will be indecomposable\footnote{We count the zero represenatation as being indecomposable (cf. definition \ref{def: (in)decomposable_objects}).} if $\calF$ itself is indecomposable.
            \end{lemma}
                \begin{proof}
                    
                \end{proof}
            \begin{corollary}[Coxeter functors act as zero on irreducible representations] \label{coro: coxeter_functors_act_as_zero_on_irreducible_representations}
                Let $\Gamma := (\Gamma_1, \Gamma_0, s, t)$ be a connected and acyclic finite quiver with an admissible enumeration of sinks:
                    $$\Gamma_0 := \{\sigma_1 \geq ... \geq \sigma_n\}$$
                The $n^{th}$ (co)Coxeter functors $\Cox_{\Gamma, \sigma_1 \geq ...\geq \sigma_n}$ and $\co\Cox_{\Gamma, \sigma_1 \geq ...\geq \sigma_n}$ then act as zero on all simple objects of $\Rep_k(\sigma_n ... \sigma_1 \Gamma)$ and those of $\Rep_k(\Gamma)$, respectively. 
            \end{corollary}
                \begin{proof}
                    Apply lemma \ref{lemma: (co)reflections_of_indecomposable_representations} to the Kronecker representations, which are together in bijection with $[\Rep_k^{\irr}(\Gamma)]$. Then, use the fact that irreducible representations of any Dynkin quiver are - up to isomorphisms - in bijection with the simple roots of said quiver (cf. proposition \ref{prop: irreducible_representations_of_dynkin_quivers_are_labelled_by_simple_roots}).
                \end{proof}
            \begin{proposition}[Coxeter functors preserve non-projective indecomposable representations] \label{prop: coxeter_functors_preserve_non_projective_indecomposable_representations}
                Let $\Gamma := (\Gamma_1, \Gamma_0, s, t)$ be a connected and acyclic finite quiver with an admissible enumeration of sinks:
                    $$\Gamma_0 := \{\sigma_1 \geq ... \geq \sigma_n\}$$
            ...
            \end{proposition}
                ...
                \end{proof}
            \begin{theorem}[Coxeter functors and Grothendieck groups] \label{theorem: coxeter_functors_and_grothendieck_groups}
                Let $\Gamma := (\Gamma_1, \Gamma_0, s, t)$ be a connected and acyclic finite quiver with an admissible enumeration of sinks:
                    $$\Gamma_0 := \{\sigma_1 \geq ... \geq \sigma_n\}$$
                Then, there are two mutually inverse $\Z$-linear operators:
                    $$
                        \begin{tikzcd}
                        	{\K_0^{\red, \not \irr}(\Gamma)} & {\K_0^{\red, \not \irr}(\Gamma)}
                        	\arrow[""{name=0, anchor=center, inner sep=0}, "{\Cox_{\Gamma, \sigma_1 \geq ...\geq \sigma_n}^{(n)}}"', bend right, from=1-1, to=1-2]
                        	\arrow[""{name=1, anchor=center, inner sep=0}, "{\co\Cox_{\Gamma, \sigma_1 \geq ...\geq \sigma_n}^{(n)}}"', bend right, from=1-2, to=1-1]
                        \end{tikzcd}
                    $$
                on the quotient $\Z$-module $\K_0^{\red, \not \irr}(\Gamma) := \K_0^{\red}(\Gamma)/\K_0^{\irr}(\Gamma)$ of the split Grothendieck group of $\Rep_k^{\fin}(\Gamma)$ by the simple Grothendieck group of the same category.
            \end{theorem}
                \begin{proof}
        ...
        \subsubsection{Dynkin quivers have finitely many isomorphism classes of indecomposable representations}
            ...
            \end{theorem}
                \begin{proof}
                    ...
                    Notice that this is already enough to show that $[\Rep_k^{\red}(\Gamma)]$ is a finite set, but we still have not yet demonstrated that isomorphism classes of indecomposable representations $[\calF] \in [\Rep_k^{\red}(\Gamma)]$ can be recovered from the positive roots $\alpha \in \Phi_{\Gamma}^+$, which entails showing that $\dim_k: [\Rep_k^{\red}(\Gamma)] \to \Phi_{\Gamma}^+$ is surjective. For this, pick an arbitrary positive root:
                        $$\alpha \in \Phi_{\Gamma}^+$$
                    which we suppose can be written as an $\N$-linear combination of the simple roots $\{\alpha_i\}_{1 \leq i \leq n}$ in the following manner:
                        $$\alpha := \sum_{1 \leq i \leq n} c_i \alpha_i$$
                    By proposition \ref{prop: irreducible_representations_of_dynkin_quivers_are_labelled_by_simple_roots}, there exist $[\calF_i] \in [\Rep_k^{\irr}(\Gamma)]$ such that:
                        $$[\calF_i] := \alpha_i$$
                    and therefore:
                        $$\alpha = \sum_{1 \leq i \leq n} c_i [\calF_i]$$
                \end{proof}

\section{Quivers, hereditary algebras, and homological dimensions \texorpdfstring{$\leq 1$}{}}
    \subsection{Quivers}
        ...
        \subsubsection{Dynkin quivers have finitely many isomorphism classes of indecomposable representations}
            ...
            Now, because we have by definition that:
                $$\Phi_{\Gamma}^+ := \{\alpha \in \N^{\Gamma_0} \mid |\alpha|_{\Gamma} = 1\}$$
            ...
            \end{theorem}
                \begin{proof}
                    ...
                    which tells us that $\dim_k: [\Rep_k^{\red}(\Gamma)] \to \Phi_{\Gamma}^+$ is injective. Notice that this is already enough to show that $[\Rep_k^{\red}(\Gamma)]$ is a finite set, but we still have not yet demonstrated that isomorphism classes of indecomposable representations $[\calF] \in [\Rep_k^{\red}(\Gamma)]$ can be recovered from the positive roots $\alpha \in \Phi_{\Gamma}^+$.
                    
                    To that end, suppose that $\alpha \in \Phi_{\Gamma}^+$ is an arbitrary positive root of $\Gamma$ and write it as:
                        $$\alpha := \sum_{1 \leq i \leq n} c_i \alpha_i$$
                    in terms of the simple roots $\alpha_i$. Using proposition \ref{prop: irreducible_representations_of_dynkin_quivers_are_labelled_by_simple_roots}, we can rewrite the equation above as:
                        $$\alpha := \sum_{1 \leq i \leq n} c_i [\calF_i]$$
                    wherein $[\calF_i] \in [\Rep_k^{\irr}(\Gamma)]$ for all $1 \leq i \leq n$.
                \end{proof}

\section{Quivers, hereditary rings, and homological dimension \texorpdfstring{$\leq 1$}{}}
    \subsection{Quivers}
        ...
        \subsubsection{(Co)reflection functors and Gabriel's Theorem}
                ...
                Let $Q := (Q_1, Q_0, s, t)$ be a quiver and $\sigma \in Q_0$ be a fixed vertex. 
                    \begin{itemize}
                        \item If $v$ is a sink, then one can define the so-called \textbf{reflection functor}:
                            ...
                            $$(\calF: [Q] \to k\mod) \mapsto \underset{v \in [Q]_{/\sigma}}{\lim} \calF(v)$$
                        \item If $v$ is a cosink, then one can instead define the \textbf{coreflection functor}:
                            $$\co\Refl_{\sigma}: \Rep_k(Q) \to \Rep_k(Q)$$
                            $$(\calF: [Q] \to k\mod) \mapsto \underset{v \in {}^{\sigma/}[Q]}{\colim} \calF(v)$$
                    \end{itemize}
            \end{definition}
            ...
            \end{remark}
            \begin{remark}[(Co)reflection functors for finite quivers are exact] \label{remark: (co)reflection_functors_for_finite_quivers_are_exact}
                If $Q := (Q_1, Q_0, s, t)$ is a finite quiver with a fixed vertex $\sigma \in Q_0$ then the corresponding (co)reflection functors:
                ...
                Now, should $Q$ be a Dynkin quiver whose vertices are enumrated by:
                    $$Q_0 := \{\sigma_1, ..., \sigma_n\}$$
                ...
                we would obtain a filtered diagram of $k$-modules:
                    $$(\co\Refl_{\sigma_n} \circ \Refl_{\sigma_n} \circ ... \circ \co\Refl_{\sigma_1} \circ \Refl_{\sigma_1})(\calF): [Q] \to k\mod$$
                and therefore we might as well assume that Dynkin quivers are filtered. Since Dynkin quivers are finite, we thus gather from the discussion in the previous paragraph that in fact, the (co)reflection functors:
                    $$\Refl_{\sigma}, \co\Refl_{\sigma}: \Rep_k(Q) \to \Rep_k(Q)$$
                are \textit{exact}. As such, if $Q := (Q_1, Q_0, s, t)$ is a Dynkin quiver and $\sigma \in Q_0$ is a fixed vertex, then the coreflection functors $\Refl_{\sigma}$ and $\co\Refl_{\sigma}$ will admit restrictions:
                    $$\Refl_{\sigma}^{\irr}, \co\Refl_{\sigma}^{\irr}: \Rep_k^{\irr}(Q) \to \Rep_k^{\irr}(Q)$$
                down onto the full subcategory $\Rep_k^{\irr}(Q) \subseteq \Rep_k(Q)$ spanned by the irreducible $k$-linear representations of $Q$.
            \end{remark}
            \begin{lemma}[Projective indecomposable objects are simple] \label{lemma: projective_indecomposable_objects_are_simple}
            ...
            \begin{theorem}[Gabriel's theorem on Dynkin quivers] \label{theorem: gabriel_theorem_on_dynkin_quivers}
                Let $k$ be a field and $Q$ be a quiver. $Q$ is then a Dynkin quiver if and only if $\Rep_k(Q)$ has finitely many isomorphism classes of indecomposable objects.
            \end{theorem}
                \begin{proof}
